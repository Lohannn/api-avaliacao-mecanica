...
const inserirNovaAvaliacao = async function (dadosAvaliacao) {
    ...
    if (dadosAvaliacao.nome == '' || dadosAvaliacao.nome == undefined || dadosAvaliacao.nome.length > 100
        ...
        if (resultdadosAvaliacao) {
            ...
            let checkTurma = await turmaDAO.selectTurmaById(novaAvaliacao[0].id_turma)
            let checkMatriculaTurma = await matriculaDAO.selectMatriculaByTurma(checkTurma[0].sigla)
            // e é isso que preciso passar na tabela intermediaria, o id das matriculas que estão na mesma turma dessa avaliação
            checkMatriculaTurma.forEach(aluno => {
                console.log(novaAvaliacao[0].idAvaliacao);
                avaliacaoDAO.insertIntoTableMatriculaAvaliacao(aluno.id, novaAvaliacao[0].idAvaliacao)
            });
